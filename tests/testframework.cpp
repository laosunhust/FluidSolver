...
UnitTest::UnitTest(const char *name, bool multi) :
		_name(name), _forge_ahead(false), _failed(false), _multi(multi), _mpiOn(
				0) {
	UnitTestDriver::s_driver().register_test(this);
}

UnitTest::UnitTest(const char *name, std::string str) :
		_name(name), _forge_ahead(false), _failed(false), _multi(0) {
	if (str != "off") {
		_mpiOn = 1;
	} else {
		_mpiOn = 0;
...
void UnitTest::assert_equal_double(double a, double b, double tol,
		const char *filename, int lineno) {
	if (fabs(a - b) <= tol)
		return;

	printf(
			"[ASSERT] %s::assert_equal_double(%.10f, %.10f, %.10f) at %s line %d\n",
...
void UnitTest::assert_equal_float(float a, float b, float tol,
		const char *filename, int lineno) {
	if (fabsf(a - b) <= tol)
		return;

	printf(
			"[ASSERT] %s::assert_equal_float(%.10f, %.10f, %.10f) at %s line %d\n",
			_name.c_str(), a, b, tol, filename, lineno);
	set_failed();
...
void UnitTest::assert_equal_int(int a, int b, const char *filename,
		int lineno) {
	if (a == b)
		return;
...
bool UnitTestDriver::run_tests(const std::vector<UnitTest *> &tests_to_run) {
	...
	for (i = 0; i < tests_to_run.size(); i++) {
		...
		if (tests_to_run[i]->is_multi() && !_multi_mode) {
			printf(
					"[ERROR] %s is a multi-gpu test, running in single-gpu mode\n",
					tests_to_run[i]->name());
			ok = false;
		} else {
			try {
...
void UnitTestDriver::print_tests() {
	for (int i = 0; i < _test_list.size(); i++) {
		if (i != 0)
			printf(", ");
		printf("%s (%s)", _test_list[i]->name(),
				_test_list[i]->is_multi() ? "multi" : "single");
	}
	printf("\n");
}
...
bool UnitTestDriver::run_MPI_enabled_tests() {
	std::vector<UnitTest *> mpi_list;
	for (int i = 0; i < _test_list.size(); i++)
		if (!_test_list[i]->is_multi()&&_test_list[i]->is_MpiOn())
			mpi_list.push_back(_test_list[i]);
	return run_tests(mpi_list);
}
...
bool UnitTestDriver::run_tests(const std::vector<std::string> &tests) {
	int i, j;
